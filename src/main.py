...
def main():
    ...
    parser.add_argument("-r", "--rest_axis", help="Axis for rests", required=True, default="X")
    parser.add_argument("--home", help="Add home command", action="store_true")
    ...
    with open(args.output, "w") as fp:
        ...
        for n, t in notes:
            if n is None:
                ...
                if rest_i == 0:
                    dist *= -1
                fp.write(f"G1 F{rate} {args.rest_axis}{dist}\n")

                rest_i = 1 if rest_i == 0 else 0

            else:
                rate = int(HALF_STEP ** (n-60) * args.pitch * 400)
                dist = (rate/60) * t * (1 if play_i == 0 else -1)
                fp.write(f"G0 F{rate} {args.axis}{dist}\n")

                play_i = 1 if play_i == 0 else 0
